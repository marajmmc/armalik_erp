...
function load_branch_name(){
    $("#branch_id").html('');
    $.post("../../libraries/ajax_load_file/load_branch_name.php", {
        bank_id: $("#bank_id").val()
    }, function(result){
        if(result){
            $("#branch_id").append(result);
        }
    })
}


function load_credit_due_balance(){
    $("#current_credit_limit").val('');
    $("#all_credit_limit").val('');
    $("#due_balance").val('');
    $("#distributor_opening_balance").val('');
    $.post("../../libraries/ajax_load_file/load_credit_due_balance.php", {
        distributor_id: $("#distributor_id").val()
    }, function(result){
        if(result){
            //            alert (result)
            var data = result.split('~');
            $("#current_credit_limit").val(data[0]);
            $("#due_balance").val(data[1]);
            $("#all_credit_limit").val(data[2]);
            $("#distributor_opening_balance").val(data[3]);
...
function distributor_due_balance(){
    ...
    $.post("../../libraries/ajax_load_file/load_distributor_purchase_value.php", {
        ...
        if(result){
            var data = result.split(':');
            $("#credit_limit_status").val($.trim(data[1]));
        }
    })
}
