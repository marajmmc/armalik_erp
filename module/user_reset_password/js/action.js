...
function Save_Rec()
{
    if($("#user_new_pass").val()!=$("#repuser_pass").val()){
        $("#user_new_pass").css("background", "#FF4A4A");
        $("#repuser_pass").css("background", "#FF4A4A");
        
        $("#user_new_pass").bind('focus', function() {
            $("#user_new_pass").css("background", "");
        });
...
function load_old_password(elm){
    $("#loader").remove();
    $.post("load_old_password.php",{
        name:elm.value
    }, function(result){
        if (result){
            if (result=="Found"){
                ...
                alertify.error("Sorry your old password don't match. try again");
                return false;
            }
        }
    });
...
function fnc_delete_request(serial,id){
    reset();
    alertify.confirm("Are You Sure .... Delete this request?", function (e) {
        if (e) {
            $.post("del_request.php", {
                elm_id: id
            }, function(result){
                if(result){
