...
for ($i = 0; $i < $count; $i++) {
    ...
    if (@$_FILES["crop_img_url"]['name'][$i] != "") {
        ...
        (
            ...
            'crop_remark' => "'" . $_POST['crop_remark'][$i] . "'",
            'entry_by' => "'$user_id'",
    ...
    if($_POST["fruit_row_id"][$j]=="")
    {
        ...
        if (@$_FILES["fruit_img_url"]['name'][$j] != "")
        {
            ...
            $rowfield = array(
                ...
                'fruit_remark,' => "'" . $_POST["fruit_remark"][$j] . "',",
...
$count_disease = count($_POST["disease_id"]);

for ($j = 0; $j < $count_disease; $j++)
{
    $disease_id = "DI-".$db->getMaxID_six_digit($tbl . 'pdo_variety_setting_disease_img', 'disease_id');

    if($_POST["disease_id"][$j]=="")
    {

        if (@$_FILES["disease_img_url"]['name'][$j] != "")
        {
            @$ext = end(explode(".", @$_FILES["disease_img_url"]['name'][$j]));
            $disease_img_url = $disease_id . "." . $ext;
            copy(@$_FILES['disease_img_url']['tmp_name'][$j], "../../system_images/pdo_upload_image/disease_img_url/$disease_img_url");

            $rowfield = array(
                'disease_id,' => "'$disease_id',",
                'vs_id,' => "'$vs_id',",
                'disease_img_url,' => "'" . $disease_img_url . "',",
                'disease_remark,' => "'" . $_POST['disease_remark'][$j] . "',",
                'upload_date,' => "'" . $db->ToDayDate() . "',",
                'status,' => "'Active',",
                'del_status,' => "'0',",
                'entry_by,' => "'$user_id',",
            ...
            $db->data_insert($tbl . 'pdo_variety_setting_disease_img', $rowfield);
            $db->system_event_log('', $user_id, $employee_id, '', '', $tbl . 'pdo_variety_setting_disease_img', 'Save', '');
        }
        else
        {
            $rowfield = array(
    ...
    else
    {
        ...
        {
            @$ext = end(explode(".", @$_FILES["disease_img_url"]['name'][$j]));
            $disease_img_url = $_POST["disease_id"][$j] . "." . $ext;
            copy(@$_FILES['disease_img_url']['tmp_name'][$j], "../../system_images/pdo_upload_image/disease_img_url/$disease_img_url");
            ...
            $where = array('disease_id'=> "'".$_POST["disease_id"][$j]."'");
            $db->data_update($tbl . 'pdo_variety_setting_disease_img', $rowfield, $where);
            $db->system_event_log('', $user_id, $employee_id, '', $_POST["disease_id"][$j], $tbl . 'pdo_variety_setting_disease_img', 'Update', '');
        }
        else
        {
            $rowfield = array
