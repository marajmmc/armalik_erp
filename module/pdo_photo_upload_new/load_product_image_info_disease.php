...
$sql = "SELECT
            ...
            $tbl"."pdo_variety_setting_disease_img.entry_date,
            $tbl"."product_type.product_type,
            $tbl"."farmer_info.farmer_name,
            $tbl"."employee_basic_info.employee_name
        FROM $tbl"."pdo_variety_setting_disease_img
             LEFT JOIN $tbl"."pdo_variety_setting ON $tbl"."pdo_variety_setting.vs_id = $tbl"."pdo_variety_setting_disease_img.vs_id
             ...
             LEFT JOIN $tbl"."pdo_product_info ON $tbl"."pdo_product_info.crop_id = $tbl"."pdo_variety_setting.crop_id AND $tbl"."pdo_product_info.product_type_id = $tbl"."pdo_variety_setting.product_type_id AND $tbl"."pdo_product_info.pdo_id = $tbl"."pdo_variety_setting.variety_id
             LEFT JOIN $tbl"."user_login ON $tbl"."user_login.user_id = $tbl"."pdo_variety_setting_disease_img.entry_by
             LEFT JOIN $tbl"."employee_basic_info ON $tbl"."employee_basic_info.employee_id = $tbl"."user_login.employee_id
        WHERE
            $tbl" . "pdo_variety_setting_disease_img.del_status='0' AND
            $tbl" . "pdo_variety_setting_disease_img.disease_id='" . $_POST['row_id'] . "'
    ";
if ($db->open())
{
    $result = $db->query($sql);
...
<div class="image_full_width">
    <img src="../../system_images/pdo_upload_image/disease_img_url/<?php echo $row['disease_img_url']; ?>" height="100%" width="100%"/>
</div>

<div style="width: 40%; float: left; ">
    <h6>
        Variety Name: <span style="color: #990033"><?php echo $row['pdo_name']; ?></span>, <br />
...
<!--    <hr />-->
    ...
    <input type="button" class="btn-info" value="Add" style="float: right;" onclick="add_disease_comment_fnc('<?php echo $row['disease_id'] ?>')"/>
</div>
<script>
    $(document).ready(function(){
        //load_disease_comment_fnc('<?php echo $row['disease_id'] ?>');
    })
</script>