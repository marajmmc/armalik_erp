...
for ($i = 0; $i < $count; $i++) {
    if ($_POST['territory_id'][$i] != "") {
        ...
        $wherefield = array('territory_id' => "'".$_POST['territory_id'][$i]."'");
        $db->data_update($tbl . 'territory_info', $rowfield, $wherefield);
        $db->system_event_log('', $user_id, $employee_id, $_POST["zone_id"], $_POST['territory_id'][$i], $tbl . 'territory_info', 'Update', '');
    } else {
