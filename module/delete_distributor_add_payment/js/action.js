...
function payment_type_fnc(){
    if($("#payment_type").val()=="Cash"){
        $("#div_cheque_no").fadeOut();
    }else{
        $("#div_cheque_no").fadeIn();
    }
}

function distributor_due_balance(){
    $("#lbl_distributor_due_balance").html('');
    $.post("../../libraries/ajax_load_file/load_distributor_due_balance.php", {
...
function payment_delete_fnc(row_id, serial){
    reset();
    alertify.confirm("Are you sure, delete this payment?", function (e) {
        if (e) {
            $.post("del_payment.php", {
                rowID:row_id
            }, function(result){
                if(result){
