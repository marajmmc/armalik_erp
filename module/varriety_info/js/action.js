...
function Existin_data(elm, event, id, crop_id, product_type_id){
    $("#loader").remove();
    $.post("exist_data.php",{name:elm.value, event:event, id:id, crop_id:crop_id, product_type_id:product_type_id}, function(result){
        if (result)
        {
            console.log(result);
            if (result=="Found")
            {
                $(elm).after('  &nbsp;  <img id="loader" src="../../system_images/loading.gif" />');
                $(elm).css('background','#ed6d49');
                MenuOffOn('off','off','off','off','off','off','on','on','on','on');
                reset();
                alertify.set({
                    delay: 3000
                });
                alertify.error("Exist Order Number. Try Again.");
                return false;
