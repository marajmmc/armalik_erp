<?php
session_start();
ob_start();
require_once("../../libraries/lib/database.inc.php");
require_once("../../libraries/lib/config.inc.php");
require_once("../../libraries/lib/functions.inc.php");
$db = new Database();
$tbl = _DB_PREFIX;


if ($_POST['zone_id'] != "") {
    $zone_id = "AND $tbl" . "employee_basic_info.zone_id='" . $_POST['zone_id'] . "'";
} else {
    $zone_id = "";
}
if ($_POST['territory_id'] != "") {
    $territory_id = "AND $tbl" . "employee_basic_info.territory_id='" . $_POST['territory_id'] . "'";
} else {
    $territory_id = "";
}
if ($_POST['employee_id'] != "") {
    $employee_id = "AND $tbl" . "weekly_tour_plan.employee_id='" . $_POST['employee_id'] . "'";
} else {
    $employee_id = "";
}
if ($_POST['from_date'] != "" && $_POST['to_date'] != "") {
    $between = "AND $tbl" . "weekly_tour_plan.plan_date BETWEEN '" . $db->date_formate($_POST['from_date']) . "' AND '" . $db->date_formate($_POST['to_date']) . "'";
} else {
    $between = "";
}
?>

<a class="btn btn-small btn-success" data-original-title="" onclick="print_rpt()" style="float: right;">
    <i class="icon-print" data-original-title="Share"> </i> Print
</a>
<div id="PrintArea" style="background-color: white;" >
    <?php include_once '../../libraries/print_page/Print_header.php'; ?>
    <table class="table table-condensed table-striped table-hover table-bordered pull-left report" id="data-table">

        <thead>
            <tr>
                <th style="width:5%">
                    Employee Name
                </th>
                <th style="width:5%">
                    Zone
                </th>
                <th style="width:5%">
                    Territory
                </th>
                <th style="width:5%">
                    Location
                </th>
                <th style="width:5%">
                    Visit Purpose
                </th>
                <th style="width:5%">
                    Initial Date
                </th>
                <th style="width:5%">
                    Visit Date
                </th>
                <th style="width:5%">
                    Feedback
                </th>
            </tr>
        </thead>
        <tbody>
            <?php
            $employee_name = '';
            $zone_name = '';
            $territory_name = '';
            $location = '';
            $visit_purpose = '';
            $entry_date = '';
            $plan_date = '';
            $sql = "SELECT
                        $tbl" . "weekly_tour_plan.id,
                        $tbl" . "employee_basic_info.employee_name,
                        $tbl" . "zone_info.zone_name,
                        $tbl" . "territory_info.territory_name,
                        $tbl" . "weekly_tour_plan.location,
                        $tbl" . "weekly_tour_plan.visit_purpose,
                        $tbl" . "weekly_tour_plan.plan_date,
                        $tbl" . "weekly_tour_plan.entry_date,
                        $tbl" . "weekly_tour_plan.purpose
                    FROM
                        $tbl" . "weekly_tour_plan
                        LEFT JOIN $tbl" . "employee_basic_info ON $tbl" . "employee_basic_info.employee_id = $tbl" . "weekly_tour_plan.employee_id
                        LEFT JOIN $tbl" . "zone_info ON $tbl" . "zone_info.zone_id = $tbl" . "employee_basic_info.zone_id
                        LEFT JOIN $tbl" . "territory_info ON $tbl" . "territory_info.territory_id = $tbl" . "employee_basic_info.territory_id
                    WHERE
                        $tbl" . "employee_basic_info.del_status='0'
                        $zone_id $territory_id $employee_id $between
            ";
            if ($db->open()) {
                $result = $db->query($sql);
                $i = 1;
                while ($result_array = $db->fetchAssoc()) {
                    if ($i % 2 == 0) {
                        $rowcolor = "gradeC";
                    } else {
                        $rowcolor = "gradeA success";
                    }
                    ?>
                    <tr class="<?php echo $rowcolor; ?> pointer" id="tr_id<?php echo $i; ?>" onclick="get_rowID('<?php echo $result_array["id"] ?>', '<?php echo $i; ?>')">
                        <td>
                            <?php
                            if ($employee_name == '') {
                                echo $result_array['employee_name'];
                                $employee_name = $result_array['employee_name'];
                                //$currentDate = $preDate;
                            } else if ($employee_name == $result_array['employee_name']) {
                                //exit;
                                echo "&nbsp;";
                            } else {
                                echo $result_array['employee_name'];
                                $employee_name = $result_array['employee_name'];
                            }
                            ?>
                        </td>
                        <td>
                            <?php
                            if ($zone_name == '') {
                                echo $result_array['zone_name'];
                                $zone_name = $result_array['zone_name'];
                                //$currentDate = $preDate;
                            } else if ($zone_name == $result_array['zone_name']) {
                                //exit;
                                echo "&nbsp;";
                            } else {
                                echo $result_array['zone_name'];
                                $zone_name = $result_array['zone_name'];
                            }
                            ?>
                        </td>
                        <td>
                            <?php
                            if ($territory_name == '') {
                                echo $result_array['territory_name'];
                                $territory_name = $result_array['territory_name'];
                                //$currentDate = $preDate;
                            } else if ($territory_name == $result_array['territory_name']) {
                                //exit;
                                echo "&nbsp;";
                            } else {
                                echo $result_array['territory_name'];
                                $territory_name = $result_array['territory_name'];
                            }
                            ?>
                        </td>
                        <td>
                            <?php
                            if ($location == '') {
                                echo $result_array['location'];
                                $location = $result_array['location'];
                                //$currentDate = $preDate;
                            } else if ($location == $result_array['location']) {
                                //exit;
                                echo "&nbsp;";
                            } else {
                                echo $result_array['location'];
                                $location = $result_array['location'];
                            }
                            ?>
                        </td>
                        <td>
                            <?php
                            if ($visit_purpose == '') {
                                echo $result_array['visit_purpose'];
                                $visit_purpose = $result_array['visit_purpose'];
                                //$currentDate = $preDate;
                            } else if ($visit_purpose == $result_array['visit_purpose']) {
                                //exit;
                                echo "&nbsp;";
                            } else {
                                echo $result_array['visit_purpose'];
                                $visit_purpose = $result_array['visit_purpose'];
                            }
                            ?>
                        </td>
                        <td>
                            <?php
                            if ($entry_date == '') {
                                echo $db->date_formate($result_array['entry_date']);
                                $entry_date = $result_array['entry_date'];
                                //$currentDate = $preDate;
                            } else if ($entry_date == $result_array['entry_date']) {
                                //exit;
                                echo "&nbsp;";
                            } else {
                                echo $db->date_formate($result_array['entry_date']);
                                $entry_date = $result_array['entry_date'];
                            }
                            ?>
                        </td>
                        <td>
                            <?php
                            if ($plan_date == '') {
                                echo $db->date_formate($result_array['plan_date']);
                                $plan_date = $result_array['plan_date'];
                                //$currentDate = $preDate;
                            } else if ($plan_date == $result_array['plan_date']) {
                                //exit;
                                echo "&nbsp;";
                            } else {
                                echo $db->date_formate($result_array['plan_date']);
                                $plan_date = $result_array['plan_date'];
                            }
                            ?>
                        </td>
                        <td><?php echo $result_array['purpose']; ?></td>
                    </tr>
                    <?php
                    ++$i;
                }
            }
            ?>
        </tbody>
    </table>
    <?php include_once '../../libraries/print_page/Print_footer.php'; ?>
</div>