...
<table class="table table-condensed table-striped table-hover table-bordered pull-left" id="data-table">
    ...
    <tr>
        ...
        <td id="territory_td_elm">
            <select id="territory_id" name="territory_id" class="span12" placeholder="Territory" onchange="load_district_fnc()" >
                <option value="">Select</option>
            </select>
        </td>
        <td id="zilla_td_elm">
            <select id="zilla_id" name="zilla_id" class="span12" placeholder="Territory" onchange="load_distributor_fnc()" >
                <option value="">Select</option>
            </select>
        </td>
        <td id="distributor_td_elm">
            <select id="distributor_id" name="distributor_id" class="span12" placeholder="Distributor" validate="Require" >
                <option value="">Select</option>
            </select>
        </td>
    </tr>
...
<script>
    ...
    {
        $("#distributor_id").html('');
        $.post("../../libraries/ajax_load_file/load_distributor.php",$("#frm_area").serialize(),
        function(result)
        {
            if (result)
            {
